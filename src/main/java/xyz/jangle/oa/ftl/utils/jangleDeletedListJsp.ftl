...
<title>${tableRemarks}_已删列表页面</title>
...
<!-- 	    </div> -->
	    <div>
	        <form class="navbar-form" role="search" id="searchForm">
	            <div class="form-group">
	            	<span class="jangle-listjsp-remarks-span label label-info" >${tableRemarks}-已删列表 </span>
	...
	<script type="text/javascript">
		...
		var url = "/${beanNameVar}Ctrl/selectPage.ctrl";	//获取数据的url地址，需要实现分页功能。
		// 新增按钮 打开新增数据的页面
		...
		function doCheckBoxes(){
			...
			if (confirm("确定删除勾选的"+rows.length+"条记录吗？删除且不可恢复")) {
				$.ajax({
					type:"POST",
					url : "/${beanNameVar}Ctrl/batchDeleteByPrimaryKeyActually.ctrl",
					dataType : "json",
					cache : false,
					data : {
						"ids" : ids
